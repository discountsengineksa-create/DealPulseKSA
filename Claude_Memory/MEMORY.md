...
- [Blog Total = Count It Live](blog_massive_content_session.md) — **١٨٢٩ (عُدَّ ٢٠٢٦-٠٩-٠٩ بعد عنقود المطار)**؛ عُدّ بـ`grep -cE "^\s*slug:" lib/blog.ts` لا تجمع تقديرياً
- [✈️ عنقود المطار للسفر](blog_almatar_travel_cluster.md) — **٣٤ مقالاً** (طيران ١٠ + تأجير سيارات ٧ + وجهات السعودية ٩ + وجهات العالم حسب الفصل ٨)؛ `master.id=86` كود **M31** خصم ٧٪ (طيران+فنادق+سيارة)؛ web `5e7aa30`/`21adfb1`/`a151695`/`fbc956f`؛ فئة جديدة `سفر وسياحة` + alias أصلح يُتم ١٤ بيد إن روم؛ **صفر رقم ريال مفبرك**؛ **بيد إن روم للسعودية فقط**؛ ⚠️ أُصلح ٤ روابط `/store/ايرالو` (بلا همزة) 404؛ غلطة «توسيع نطاق» (de-orphan أصاب ٢٦٤ لا ٥ — رُوجع)
