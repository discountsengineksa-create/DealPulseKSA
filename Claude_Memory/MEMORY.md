...
> ١٤٤ ملفاً (عُدّت ٢٠٢٦-٠٨-٢٣ بعد منظومة الحملات — `ls *.md | grep -v MEMORY.md | wc -l` = ١٤٤، وفحص الاكتمال: صفر مفقود). **العدّ للمتون، بلا `MEMORY.md` نفسه** (`ls *.md` يعطي ١٤٥ — لا تنسخه هنا). **الفهرس وحده يُحمَّل — المتون لا.** السطر هنا **عنوان لا حقيقة**؛ الحقيقة في المتن.
