...
> ١٦٤ ملفاً (عُدّت ٢٠٢٦-٠٩-١٠ بعد عنقود رسيس — `ls *.md | grep -v MEMORY.md | wc -l` = ١٦٤). **العدّ للمتون، بلا `MEMORY.md` نفسه**. **الفهرس وحده يُحمَّل — المتون لا.** السطر هنا **عنوان لا حقيقة**؛ الحقيقة في المتن.
...
## ٦) المحتوى والمدوّنة (٢٥)

- [Voice Bible](voice_bible.md) — نموذج الصوت التحريري؛ قلّد العيّنة
- [Blog Total = Count It Live](blog_massive_content_session.md) — **١٨٥٥ (عُدَّ ٢٠٢٦-٠٩-١٠ بعد عنقود رسيس)**؛ عُدّ بـ`grep -cE "^\s*slug:" lib/blog.ts` لا تجمع تقديرياً
