...
> ١٦١ ملفاً (عُدّت ٢٠٢٦-٠٩-٠٩ بعد عنقود دبدوب — `ls *.md | grep -v MEMORY.md | wc -l` = ١٦١). **العدّ للمتون، بلا `MEMORY.md` نفسه**. **الفهرس وحده يُحمَّل — المتون لا.** السطر هنا **عنوان لا حقيقة**؛ الحقيقة في المتن.
...
- [Blog Total = Count It Live](blog_massive_content_session.md) — **١٧٩٥ (عُدَّ ٢٠٢٦-٠٩-٠٩ بعد عنقود دبدوب)**؛ عُدّ بـ`grep -cE "^\s*slug:" lib/blog.ts` لا تجمع تقديرياً
