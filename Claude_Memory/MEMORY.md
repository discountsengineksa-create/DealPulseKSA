...
> ١٦٣ ملفاً (عُدّت ٢٠٢٦-٠٩-٠٩ بعد عنقود المطار — `ls *.md | grep -v MEMORY.md | wc -l` = ١٦٣). **العدّ للمتون، بلا `MEMORY.md` نفسه**. **الفهرس وحده يُحمَّل — المتون لا.** السطر هنا **عنوان لا حقيقة**؛ الحقيقة في المتن.
...
## ٦) المحتوى والمدوّنة (٢٤)

- [Voice Bible](voice_bible.md) — نموذج الصوت التحريري؛ قلّد العيّنة
- [Blog Total = Count It Live](blog_massive_content_session.md) — **١٨٣٩ (عُدَّ ٢٠٢٦-٠٩-٠٩ بعد عنقود المطار)**؛ عُدّ بـ`grep -cE "^\s*slug:" lib/blog.ts` لا تجمع تقديرياً
