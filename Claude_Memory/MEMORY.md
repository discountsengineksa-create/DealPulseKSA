...
- [📈 GA4 رُكِّب — ومربوط بمفاتيح اللوحة](web_ga4_install.md) — `G-VRBHD0VK66` حيّ على ٦ مسارات؛ **لا `page_view` يدويّ فالقياس المحسّن هو من يرصد تنقّل Next**؛ وCSP كانت ستحجبه بصمت؛ و**قلب مفتاح «إشارات Google» يُلزِم تعديل نصّ الخصوصية في نفس الجلسة**؛ والدليل التنفيذي `seo/ga4_playbook.md` (١١ مهمة مصنَّفة + BigQuery تأميناً + GA4 أعمى عن قناة الـAI)
