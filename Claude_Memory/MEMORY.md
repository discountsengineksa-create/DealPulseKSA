...
> ١٤٩ ملفاً (عُدّت ٢٠٢٦-٠٨-٢٦ بعد ملف مصادر لائحة النقل — `ls *.md | grep -v MEMORY.md | wc -l` = ١٤٩). **العدّ للمتون، بلا `MEMORY.md` نفسه** (`ls *.md` يعطي ١٤٩ — لا تنسخه هنا). **الفهرس وحده يُحمَّل — المتون لا.** السطر هنا **عنوان لا حقيقة**؛ الحقيقة في المتن.
