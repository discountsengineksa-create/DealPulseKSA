...
> ١٤٥ ملفاً (عُدّت ٢٠٢٦-٠٨-٢٤ بعد ملف حرق التوكنز — `ls *.md | grep -v MEMORY.md | wc -l` = ١٤٥، وفحص الاكتمال بالاتجاهين: صفر مفقود). **العدّ للمتون، بلا `MEMORY.md` نفسه** (`ls *.md` يعطي ١٤٦ — لا تنسخه هنا). **الفهرس وحده يُحمَّل — المتون لا.** السطر هنا **عنوان لا حقيقة**؛ الحقيقة في المتن.
