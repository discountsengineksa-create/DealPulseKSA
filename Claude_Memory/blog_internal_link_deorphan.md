...
## ٢٠٢٦-٠٨-٢٦ — الباب المحايد، وفخّان في طريقه

**الباب المحايد بدل غزو عنقود شريك:** جيني (مشاوير) كان يحتاج مدخلاً من المدوّنة،
وكل روابط موضوع «التوصيل» تعيش داخل عنقود تويو (١٥ مقالاً، ملك شريك). فبدل زرعه هناك
— وهو ما يمنعه [[blog_nazih_cluster]] — كُتب مقال **محايد** `delivery-ride-apps-saudi-2026`
(«تطبيقات التوصيل والمشاوير») يشرح الفرق بين تطبيق يوصّل **طلبك** وتطبيق يوصّلك **أنت**،
ويربط تويو + جيني + نون معاً. ودخلت روابط جيني منه ومن هَبّين محايدين
(`best-saudi-online-stores-2026` بقسم جديد، و`ramadan-deals-guide`). ومقال تويو الهَب
يشير إلى **الدليل المحايد لا إلى صفحة المنافس**.
الحصيلة المقيسة حيّاً: **١٦ رابطاً من ١١ صفحة** إلى `/store/جيني`، والمقال الجديد
**٣ روابط داخلة** (لا يتيم).

**فخّ ١ — `extractFaq` يشترط أن يكون السؤال كتلة مستقلّة.** الشرط ليس وجود العنوان
`## أسئلة شائعة` (هكذا وُصف سابقاً وهو ناقص): الدالة تقسم القسم على **سطر فارغ**،
وتشترط أن تطابق الكتلة `^\*\*(.+)\*\*$` **وحدها**، وأن تليها كتلة الجواب.
فسؤالٌ وجوابه في سطرين متلاصقين = **صفر عقدة Question** بلا أي خطأ.
حدث فعلاً: المقال نُشر ٢٠٠ وبلا `FAQPage`، وبعد إضافة سطر فارغ بين كل سؤال وجوابه
صارت **٤ عقد Question** (مؤكَّد على الصفحة الحيّة).

**فخّ ٢ — هَب `/stores` كان يربط ٤٨ من ٥٨.** الشبكة مكوّن عميل بـ`PAGE_SIZE = 48`
وزر «المزيد»، وفرزه الافتراضي **بالشعبية** — فالمتجر الجديد (شعبية صفر) يقع خلف
الصفحة الأولى فلا رابط له في أول HTML أصلاً، ومعه سقف `itemListSchema` كان ٥٠.
العلاج: **فهرس نصّي لكل المتاجر مُصيَّر على الخادم** أسفل الشبكة (web `8d5bff5`) —
روابط المصدر **٦٨ ← ١٢٥** ورابط المتجر الجديد ظهر (مقيس بعد النشر).
⇒ **قاعدة: بعد إضافة أي متجر، عُدَّ `href="/store/` في مصدر `/stores` قبل افتراض أنه موصول.**

**فخّ ٣ — تصنيف المقال الخام يسقط بصمت إلى `savings-guides`.** `lib/blog.ts` يترجم
`category` النصّي إلى **سلَگ قانوني** عبر `CATEGORY_ALIASES`، و`categorySlugOf` ترجع
`'savings-guides'` لأي وسم غير مُخرَّط. فعنقود المشاوير الثمانية دخل «أدلّة التوفير»
بلا خطأ، و`/blog/category/…` بالاسم العربي **404** لأن المسار يستعمل السلَگ الإنجليزي.
⇒ **أي تصنيف جديد يحتاج سطرين: عقدة في `BLOG_CATEGORIES` وسطراً في `CATEGORY_ALIASES`**
— ثم تحقّق بـ`getBlogCategories()` أن العدد ظهر (صار `rides` = ٨). المصادر في
[[ride_hailing_regulation_sources]].
