...
- البوت (deal_pulse_bot.py — نُفِّذ آنذاك بإذن لمرة واحدة أيام التجميد؛ **التجميد رُفع ٢٠٢٦-٠٧-٠٧** [[bot_frozen_lock]]): fetch_api_results يمرّر channel=bot؛ _db_search/listings/favorites تفلتر `ILIKE '%bot%'`؛ + fallback جديد `_db_search_website_exclusive` → البحث عن متجر حصري للموقع يردّ زر «حصري بالموقع» برابط `WEBSITE_URL/store/{store_id}`.
