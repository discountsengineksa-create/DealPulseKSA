...
**⚠️ مصدر ثانٍ للحقيقة داخل المتون (رُصد ٢٠٢٦-٠٨-١٧):** بعض المتون تكتب الرقم والكود يدوياً بصيغة «بخصم ‏X%‏ بكود ‏Y‏» بجانب الشارة التي ترسم نفس الحقلين حيّاً — **٦٢ موضعاً / ٣ أكواد فقط** (`R1IMJ` ٥٠ · `XCCGA` ١١ · `CMM236` ١). انفجرت فعلاً: غيّرت سلة عرض **جي كارد** فبقيت ٥٠ مقالة تعلن `CM2610` و«5%» بعد موتهما. الفحص الدوري سطر واحد — `grep -oE 'بخصم [^ ]+ بكود [A-Za-z0-9_-]+' lib/blog.ts` ثم قارن بـ`master`. العلاج الجذري (غير منفَّذ): حذف الجملة من المتون بعد توسيع الشارة لتحمل نسبة الخصم — تغيير عارض يمسّ كل ١٣٦٥ مقالة، فيحتاج قرار المالك لا تمريراً ضمنياً.
