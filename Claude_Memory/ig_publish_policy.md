...
- منذ **2026-07-15** صار المسار **Reels-only**: `IG_AUTO_FEED_ENABLED` مطفأ، فالـdispatcher **لا يُدرج صف `instagram` أصلاً** في `social_posts_log` (لا sent ولا failed ولا skipped). المتاجر القديمة (ديزل/سبورتر/بيد إن روم، يوليو ١٢-١٤) لها صفوف `instagram=sent` لأنها تسبق التغيير — فلا تقارن بها.
- ⚠️ **هذا السطر بطل جزئياً:** كاروسيل الفيد **أُعيد تفعيله ٢٠٢٦-٠٨-٠٢** بقرار المالك، والافتراضي في الكود صار **مُشغَّلاً** — `dispatcher.py:34` يقرأ `os.getenv("IG_AUTO_FEED_ENABLED", "1")`، أي الفيد يعمل ما لم تُضبط البيئة على `0` (تحقّق ٢٠٢٦-٠٨-١٧ بقراءة السطر). فالبثّ اليدوي/التلقائي **ينشر بوستر الفيد على إنستقرام** ويُدرج صف `instagram`. الريل الجماعي مستقلّ ويبقى يعمل.
