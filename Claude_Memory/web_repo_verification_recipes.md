...
metadata: 
  ...
  modified: 2026-09-14T20:29:49.124Z
---
...
**⚠️ تحديث ٢٠٢٦-٠٩-١٤ — السقف ٨GB انكسر مع نموّ الموقع:** بعد نموّ الكتالوج من ١٬٦١٧ إلى
٢٬٠٤٢ مقالاً (٢٬٤٣٥ صفحة سِتاتيك)، `--max-old-space-size=8192` **فشل فعلياً** —
`EXIT=134` + `FATAL ERROR: AlignedAlloc Allocation failed - process out of memory`
(السجل، لا الإشعار — إشعار المهمّة الخلفية ادّعى نجاحاً على نفس هذا البناء الفاشل مرّة
أخرى، نفس الفخّ المسجَّل في ٢٠٢٦-٠٨-١٨). **`--max-old-space-size=15000` أكمل: `EXIT=0`
في ٣٫٤ دقيقة** على جهاز بـ٧٩GB رام فعلية. ⇒ **الرقم ٨١٩٢ ليس ثابتاً — يعتمد على حجم
`lib/blog.ts` وعدد الصفحات وقت التشغيل.** ابدأ بـ٨GB؛ لو فشل بـ`out of memory` صريحة
(لا بمهلة زمنية) ارفع إلى ١٥٠٠٠ مباشرة بدل تجربة قيم وسيطة.
...
### 🔴 ولماذا الإيقاف ليس نظافةً بل صحّةَ قياس (٢٠٢٦-٠٨-١٨)

خادمٌ متبقٍّ من جلسة أسبقَ على 3100 **يزيّف النجاح**: أمري الجديد مات بـ`EADDRINUSE`
(كود ٢) بينما `curl` نجح فوراً — فظننتُ أنه خادمي. الخادم القديم **يقرأ HTML الجديد من
القرص** (فظهر الوسم الجديد فعلاً) **لكن `next.config.mjs` يُقرأ عند الإقلاع** فأجاب برأس
**CSP القديم بلا نطاقات GA4**. لو صُدِّق لأُعلن «CSP مضبوط» وهو محجوب في المتصفّح.

⇒ **قبل أي فحص على 3100: اقرأ سجل الخادم للتأكّد أنه أقلع فعلاً (`✓ Ready in …`)، لا أن
`curl` رجّع ٢٠٠.** والقتل: `Get-NetTCPConnection -LocalPort 3100 -State Listen` ثم
`Stop-Process -Force`. (تفاصيل التركيب في [[web_ga4_install]].)
...
## 🔴 الوصفة الضيّقة **لا تغطّي `lib/blog.ts`** — وهذا ما كسر الإنتاج (٢٠٢٦-٠٨-٢٥)

الفقرة ١ أعلاه تحصر `include` في الملفات المتغيّرة **بشرط ألا تستورد `lib/blog.ts`**. النتيجة
العملية التي لم تكن مكتوبة: **أكثر ملف نعدّله هو الملف الوحيد الذي لا يُفحص أبداً.**

**ما حدث:** جلسة أخرى ألحقت ١٤ مقال نزيه داخل `BLOG_CATEGORIES`
(نوعها `{slug,name,name_en,emoji}`) بدل مصفوفة `posts`. النتيجة:

```
lib/blog.ts(130221,5): error TS2353: 'title' does not exist in type 'BlogCategory'
... ١٤ خطأ · Failed to type check · EXIT=1
```

**والأثر أسوأ من خطأ بناء عادي:** Vercel توقّف عن النشر من ذلك الكوميت، **والموقع ظل يخدم
البناء الأقدم بلا أي عَرَض ظاهر للزائر**. كل ما دُفع بعده — ١٨ رابطاً بين مقالات جديدة وقسم
تقسيط وخريطة مزوّدين — كان في git ولا شيء منه على الإنتاج. ومقالات نزيه نفسها **لم تُنشر
أصلاً** لأنها كانت في المصفوفة الغلط. اكتُشف فقط لأن صفحة جديدة رجعت 404 بعد ٢٠ دقيقة.

**القاعدة الجديدة:** بعد أي تعديل على `lib/blog.ts` شغّل **البناء الكامل** لا الـtsconfig الضيّق:

```bash
NODE_OPTIONS=--max-old-space-size=8192 npx next build     # ~2 دقيقة، EXIT=0
# ✓ Compiled successfully · ✓ Generating static pages (1865/1865) in 49s
```

`node --check` على الكائن الجديد وحده **لا يكفي** — الكائن سليم نحوياً وخطؤه في **النوع**
وفي **المصفوفة التي وُضع فيها**. وكذلك عدّ الـslug لا يكشفه (يعدّ الغلط كصحيح).

**وقاية أرخص من البناء:** بعد أي إلحاق بـ`blog.ts` تأكّد أن الكائن دخل مصفوفة `posts`
(تبدأ عند `export const posts: BlogPost[] = [` وتُغلق قبل `export interface BlogCategory`)
وأن عدد `BLOG_CATEGORIES` بقي **٢١**.

## ⚠️ جلستان على ريبو واحد

هذا الحادث ناتج عن جلستين تكتبان `lib/blog.ts` (٦ ميجا) في وقت واحد بلا قفل. `git status`
كان نظيفاً والدفع نجح — التصادم كان **دلالياً لا نصّياً**. عند العمل بجلستين: افحص
`git log --oneline -5` قبل البدء، وشغّل البناء الكامل قبل الدفع.
