...
**متى:** ٢٠٢٦-٠٨-٠٩. المالك سجّل حساباً على `saashub.com` (دليل بدائل برمجيات، ٢٣٦ ألف منتج).
**الحالة وقتها:** الحساب موجود، **المنتج غير مُدرَج** — بحث `dealpulseksa` هناك يرجّع صفحة
نتائج فاضية (الكلمة تظهر في `<title>` الاستعلام لا في نتيجة).
...
**المعلّق:** وسم الـmeta من `saashub.com/verify/<slug>` بعد الدخول ⇒ يُضاف في
`app/layout.tsx` داخل `metadata.verification.other` (نفس نمط Admitad/Bing، السطر ١٠٥).
