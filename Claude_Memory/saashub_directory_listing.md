...
## 🏆 الجائزة كانت في حقل جانبي لا في صفحة المنتج

`Details ← Startup Details` يُدرجك مجاناً في **`saashub.com/startups/saudi-arabia`**
(«Top 42 Startups based in Saudi Arabia»). روابط ذلك الدليل **بلا سمة `rel` إطلاقاً**
(`<a target="_blank" href="https://www.notah.ai/">`) ⇒ **dofollow**، والصفحة **سعودية**
لا إنجليزية عامة. تكلفتها ٤ حقول (Country/City/Employees/Founders).
**الدرس:** لا تحكم على قيمة دليل من صفحته الرئيسية — أعلى قيمة فيه كانت في تبويب فرعي
داخل نموذج التحرير، ما كنتُ لأجدها لولا أن المالك مرّ على النموذج كاملاً.

## التوثيق ≠ الاعتماد (تصحيح نموذج)

«verified ⇒ dofollow» صحيحة **بعد الاعتماد فقط**. بعد نجاح `Verify` بقيت الصفحة
`Not approved` و**صفر روابط** لنطاقنا. الترتيب: **Approved ⇒ يظهر رابط الموقع أصلاً ·
Verified ⇒ يصير dofollow.** التوثيق بيدنا، الاعتماد قرارهم.

## ⚠️ إدخال منافس غير مُدرَج يفتح له صفحة

`almowafir` بعد إدخاله صارت `saashub.com/almowafir` تفتح 200 (Not approved، بلا رابط لهم،
وبلا ذكر لنا)، بينما `alcoupon`/`sahseh`/`voucherek`/`almotasuq`/`shwerni` بقيت **404** ولم
تُحتسب — الصفحة العامة تعترف بالستة الإنجليز فقط. ⇒ صفر مكسب لنا وصفحة مجانية للمنافس.
**حقل المنافسين يقبل الموجود عندهم فقط؛ ما عداه إمّا يُهمَل أو ينشئ إدراجاً لخصمك.**
