...
- ⚠️ **`my_coupon='6%'`** — نفس غلط ناتشورال تاتش/ريف: نسبة العمولة في خانة كود التتبّع. **كتابة DB، معلّقة على إذن المالك.**
...
- **`my_coupon='6%'` → يجب أن يكون كود عمولة أو يُفرَّغ** (كتابة DB، إذن المالك).
- **`blog_bridge`** — لم يُعَد بناؤه (كتابة DB). النهدي + 20 مقالاً غير مربوطين في البحث الذكي بعد.
