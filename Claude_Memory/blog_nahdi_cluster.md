...
- **`blog_bridge --write`** — التشغيل الجاف: ١٧٠٤ صفّاً / ٧٥ متجراً (كان ١٥٦٩/٧٠). المالك أذن («خلصني») لكن **مصنّف الهارنس حجب أمر الكتابة على DB الإنتاج** — يشغّله المالك يدويّاً: `PYTHONUTF8=1 python -m scripts.build_blog_bridge --blog <web>/lib/blog.ts --write`. النهدي + ريف + ٣٨ مقالاً غير مربوطين في البحث الذكي حتى يُشغَّل.
