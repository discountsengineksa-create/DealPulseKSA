...
**🚪 صفحات الدخول قابلة للفتح + إسناد التفاعل (2026-08-01):** `landing_path` يُخزَّن **مُرمَّزاً** (`/store/%D8%AC%D9%8A%20…`) لأن أسماء المتاجر/الأقسام عربية — فالعرض الخام غير مقروء. الداشبورد الآن يعرض المسار مفكوك الترميز (`unquote`) + عمود رابط منفصل «🔗 افتح» (LinkColumn **لا يدعم نصاً مخصّصاً لكل صف** — فقط ثابت أو regex على الرابط، لذلك فُصل النص عن الرابط) + عمود «نوع الصفحة» من أول مقطع (blog/store/category/c/calendar…). وقسم «صفحات الدخول» يعطي لكل صفحة: زيارات/زوّار + من وين جاءوا + ماذا فعلوا بعدها، وفحصاً لصفحة واحدة (مصادرها/مدنها/حركاتها + جلب كلمات GSC لتلك الصفحة بفلتر `page equals`).

**نموذج الإسناد:** كل حدث في `action_logs` يُنسب لآخر `web_visits.landing_path` سبق `action_time` لنفس الـ`visitor_id` (subquery `ORDER BY created_at DESC LIMIT 1`) — لا تكرار للعدّ عبر الصفحات، بخلاف الربط المباشر visitor↔path.

**⛔ السقف — لا pageviews:** الموقع يُطلق ping **واحداً لكل جلسة**، فالمعروف هو **صفحة الدخول فقط**؛ مسار التصفّح الكامل غير مُسجَّل. عمود «صفحات تصفّحها» = عدد أحداث action_logs (view_store/نقر/نسخ/بحث) لا عدد صفحات. لمعرفة المسار الكامل يلزم تعديل في ريبو الويب (ping على تغيّر المسار) — [[reconcile_web_repo_separately]]. وكلمة البحث نفسها لا تصل أبداً بالإحالة (قوقل يُجرّدها) — مصدرها الوحيد Search Console.
