            ...
            if missing:
                ...
                try:
                    ...
                    cur.execute("""
                        ...
                        VALUES (%s,%s,%s,%s, %s,%s,%s,%s, %s,%s,%s,%s, %s,%s,%s, 0,0,'عادي')
                    """, (
...
if page == "تحليل الموقع":
    ...
    if not compare_df.empty:
        ...
        fig_cmp = px.bar(compare_df, x="المصدر", y="الأحداث",
                            title="توزيع الأحداث بحسب المصدر",
                            color="المصدر",
    ...
    if not growth_df.empty:
        ...
        with col_g1:
            fig_day = px.bar(growth_df, x="التاريخ", y="مسجّلون جدد",
                                title="التسجيلات اليومية",
                                color_discrete_sequence=["#10B981"])
            st.plotly_chart(fig_day, use_container_width=True)
        with col_g2:
            fig_cum = px.line(growth_df, x="التاريخ", y="الإجمالي التراكمي",
    ...
    if not activity_df.empty:
        fig_act = px.pie(activity_df, names="الفئة", values="العدد",
    ...
    events_df = pd.read_sql("""
        SELECT
            action_type                  AS "نوع الحدث",
            COUNT(*)                     AS "العدد",
            COUNT(DISTINCT store_id)     AS "عدد المتاجر",
            DATE(MIN(action_time))       AS "أول حدث",
            DATE(MAX(action_time))       AS "آخر حدث"
        FROM action_logs
        WHERE source = 'web'
    ...
    if not events_df.empty:
        fig_ev = px.pie(events_df, names="نوع الحدث", values="العدد",
    ...
    if not top_web.empty:
        fig_top = px.bar(top_web.head(10), x="المتجر", y="الأحداث",
    ...
    if not time_ev.empty:
        fig_te = px.line(time_ev, x="التاريخ", y=["نسخ", "نقرات"],
    ...
    """, conn)
    if not top_kw.empty:
        fig_kw = px.bar(top_kw.head(15), x="الكلمة", y="عدد البحث",
    ...
    gaps_df = pd.read_sql("""
        ...
        FROM direct_search
        WHERE platform = 'Web' AND user_found = false
        GROUP BY search_keyword
        ORDER BY COUNT(*) DESC
        LIMIT 20
    """, conn)
    ...
    with col_c1:
        if not city_df.empty:
            fig_city = px.bar(city_df, x="المدينة", y="العدد",
    ...
    with col_c2:
        if not country_df.empty:
            fig_cnt = px.pie(country_df, names="الدولة", values="العدد",
                                title="توزيع الدول",
                                color_discrete_sequence=px.colors.qualitative.Set2)
    ...
    if not device_df.empty:
        fig_dev = px.pie(device_df, names="الجهاز", values="العدد",
