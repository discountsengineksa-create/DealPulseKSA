...
if page == "إدخال بيانات الماستر":
    ...
    with st.form("master_final_form", clear_on_submit=False):
        ...
        st.caption(
            "علّم القنوات المسموح بها لهذا المتجر. لو منع المعلن تيليجرام "
            "(مثل SHEIN/Airalo) شِل صحّ «البوت» — يبقى حصرياً على الموقع. "
            "«📧 البريد» = السماح بحملة بريد **باسم المتجر**؛ شِله للي يمنع الإيميل "
            "(نرسل لمفضّليه إشعاراً عاماً بلا اسمه)."
        )
        ...
        try:
            ...
            if res:
                with st.form("edit_master_arabic_columns"):
                    ...
                    st.markdown("**📤 إرسال إلى (قنوات النشر)**")
                    st.caption(
                        "علّم القنوات المسموح بها للمتجر. شِل «البوت» للمتاجر التي "
                        "يمنعها المعلن من تيليجرام. «📧 البريد» = حملة بريد باسم المتجر؛ "
                        "شِله للي يمنع الإيميل (يُرسَل لمفضّليه إشعار عام بلا اسمه)."
                    )
    ...
    with sm_tab_main:
        ...
        if not df_logs.empty:
            ...
            agg = (ev.groupby(_grp_keys, dropna=False)
                     .agg(copy=("copy", "sum"),
                          click=("click", "sum"),
                          copy_trend_daily=("copy_trend_daily", "sum"),
                          ...
                          click_story_trend=("click_story_trend", "sum"),
                          srch=("srch", "sum"),
        ...
        if agg.empty:
            ...
            agg = agg.rename(columns={
                ...
                "copy":                "نسخ",
                "click":               "نقر",
                "copy_trend_daily":    "نسخ ترند يومي",
    ...
    if not _ov_df.empty:
        ...
        if _promoted_now:
            ...
            _rm_sel = _rm_c1.multiselect(
                "🗑️ إلغاء الإشهار العادي", _promoted_now,
                key="story_unpromote_sel",
                help="يلغي إشهار المتجر العادي فقط (الطبقة الثالثة). لو المتجر ضمن "
                     "الترند اليومي/الأسبوعي يبقى في صف الستوري تلقائياً. لا يحذف "
                     "المتجر ولا شرائحه.")
            with _rm_c2:
                st.write(""); st.write("")
        ...
        if journey.empty:
            ...
            if sv_agg_mode in ("store_total", "customer_total"):
                ...
                if sv_agg_mode == "store_total":
                    ...
                    # المجاميع تشمل الترند والعادي معاً (لو فلتر المشاهدات = الكل).
                    # حالة الستوري لكل متجر: لو في ترند + عادي → نُظهر «🔥 + 🎬»
                    # (أمانة في العرض) بدل ما نخفي أحدهما.
                    def _trend_mix(s):
                        has_t = (s == "🔥 ترند").any()
                        has_n = (s == "🎬 عادي").any()
                        has_u = (s == "— غير معروف").any()
                        if has_t and has_n: return "🔥 ترند + 🎬 عادي"
                        if has_t:           return "🔥 ترند"
                        if has_n:           return "🎬 عادي"
                        if has_u:           return "— غير معروف"
                        return "—"
                    _agg_spec = {
                        "المصدر":         _src_join,
                        "حالة_الستوري":   _trend_mix,
                        "مرات_المشاهدة":  "sum",
                        "دخل_المتجر":     _yes,
                        "عدد_الزيارات":   "sum",
                        "نسخ_الكود":      _yes,
...
# --- إدارة الموقع: أعلام تشغيل عامّة يقرأها الويب (platform_settings) ---
    ...
    if _gate_on:
        ...
        st.warning(
            "🔴 **البوّابة مُطفأة حالياً** — الموقع مفتوح للجميع: الأكواد والزيارة "
            "تظهر بلا تسجيل. **الستوري والمفضلة تبقى للمسجّلين فقط** (وصفحات/زر "
            "تسجيل الدخول تبقى متاحة عشانها). استخدمها في مرحلة بناء الزيارات، ثم "
            "أعِد تفعيلها لمّا توصل أرقام مطلوبة من البراندات."
        )

    st.markdown(
        "- **مفعّلة** = نجمع حسابات (مطلوب لشروط بعض البراندات)، لكن نخسر زوّاراً "
        "متردّدين.\n"
        "- **مطفأة** = احتكاك شبه صفري: كل زائر يشوف الكود فوراً (تبقى الستوري "
        "والمفضلة خلف التسجيل) — الأنسب الآن لرفع الزيارات بسرعة.\n"
        "- في الحالتين: حركات الزائر المجهول تُحتسب في التحليلات عبر بصمة "
        "الزائر (visitor_id)."
    )
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    # أكثر الزوّار تكراراً (نفس الشخص دخل كم مرة) — عدّ الجلسات فقط.
    # التعرّف الكامل على سلوك المجهول (نسخ/نقر/بحث) يعيش في الأقسام الموجودة
    # (تحليل المستخدمين/المتاجر/الأقسام) عبر COALESCE(user_id, visitor_id) — لا جدول موازٍ.
