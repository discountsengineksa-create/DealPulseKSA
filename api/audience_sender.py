...
def list_schedules(conn, *, enabled_only: bool = False) -> list[dict]:
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            f"s.channel, s.schedule_type, "
            f"(s.run_at       AT TIME ZONE 'Asia/Riyadh')::text AS run_at, "
            f"s.cron_expr, s.enabled, "
            f"(s.last_run_at  AT TIME ZONE 'Asia/Riyadh')::text AS last_run_at, "
            f"(s.next_run_at  AT TIME ZONE 'Asia/Riyadh')::text AS next_run_at, "
            f"(s.created_at   AT TIME ZONE 'Asia/Riyadh')::text AS created_at "
